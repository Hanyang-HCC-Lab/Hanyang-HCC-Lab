...
날짜 표기는 기존 관례인 `[YYYY.MM] 설명` 형식을 권장합니다. `image`는 대표 사진이자 기존 데이터와의 호환을 위한 필드입니다. 한 이벤트에 사진이 여러 장이면 선택 사항인 `images` 배열에 표시 순서대로 주소를 넣고, 첫 주소를 `image`와 동일하게 유지합니다. 공개 사이트는 여러 장일 때만 좌우 버튼과 모바일 스와이프를 표시하며 자동 재생하지 않습니다. 관리자에서는 여러 파일을 한 번에 올린 뒤 대표 사진과 순서를 바꿀 수 있습니다. 한 장뿐인 기존 항목은 `images` 없이 그대로 사용할 수 있습니다.
